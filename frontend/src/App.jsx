...
export default function App(){
 const [token,setToken]=useState('');const [account,setAccount]=useState(null);const [login,setLogin]=useState(false);const [email,setEmail]=useState('');const [loginBusy,setLoginBusy]=useState(false);const [loginNotice,setLoginNotice]=useState('');const [authReady,setAuthReady]=useState(false);const [modelStatus,setModelStatus]=useState('');
 ...
 useEffect(()=>{
  ...
  function apply(session){
   ...
   if(session){setLogin(false);setLoginNotice('')}
  }
 ...
 async function connect(e){
  e.preventDefault();setLoginBusy(true);setError('');setLoginNotice('');
  try{
   if(!auth)throw Error('Email sign-in is being set up. Please try again later.');
   const {error}=await auth.auth.signInWithOtp({email:email.trim(),options:{emailRedirectTo:window.location.origin+'/'}});
   if(error)throw error;
   setLoginNotice('Check your email for a sign-in link. If it has expired, request a new one. You can close this window after opening the link.');
 ...
 {login&&<div className="shade"><form className="modal" role="dialog" aria-modal="true" aria-labelledby="login-title" onSubmit={connect}><button type="button" className="close" aria-label="Close" onClick={()=>setLogin(false)}><X/></button><AudioLines className="mint" size={30}/><h2 id="login-title">A space of your own.</h2><p>Sign in with your email. We’ll send you a secure link—no password or access code needed.</p><label htmlFor="email">Email address</label><input id="email" autoFocus type="email" autoComplete="email" placeholder="you@example.com" value={email} onChange={e=>setEmail(e.target.value)} required maxLength={254}/>{loginNotice&&<p role="status">{loginNotice}</p>}{!auth&&<p role="status">Email sign-in is not available yet. The owner is finishing setup.</p>}{error&&<p role="alert" className="login-error">{error}</p>}<button className="wide" disabled={loginBusy||!auth}>{loginBusy?'Sending link…':'Email me a sign-in link'}<ArrowUpRight size={17}/></button></form></div>}
 </div>
}
